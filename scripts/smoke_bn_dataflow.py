...
def main() -> int:
    ...
    print(f"[+] {len(cs.call_sites)} total call sites; "
          f"{len(cs.schema_emittable())} schema-emittable")

    targets = cs.for_api(target_api)
    if not targets:
        print(f"[FAIL] Unit 3 found no call sites to {target_api}. "
              f"Nothing for the bridge to work on. APIs present include: "
              f"{sorted(list(cs.api_names()))[:15]} ...")
        return 1
    for s in targets:
        print(f"      {target_api}  call_site=0x{s.call_site_va:08x}  "
    ...
    if expect is not None:
        ...
        print(f"[{'PASS' if ok else 'FAIL'}] expected substring {expect!r} "
              f"in a resolved {target_api} argument")
        return 0 if ok else 1
    if resolved_strings:
        print(f"[PASS] bridge resolved a string argument to {target_api}")
        return 0
    print(f"[WARN] {target_api} call located but no argument resolved to a "
          f"string. Triage with the per-record line above:")
    print("       value=None, path=[call_va only]  -> arg was a live-in/return "
    ...
    print("       no record at all                  -> _find_ssa_call op names "
          "(MLIL_*_SSA) don't match this build.")
    print("       value=None but path has hops      -> reached a CONST that isn't "
