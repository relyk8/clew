...
- **Local copy**: not committed. To rebuild: open `al-khaser.sln` in Visual
  Studio 2022 (Desktop development with C++, toolset v143), select the `Debug`
  configuration and the `x86` platform, and build the `al-khaser` project. The
  binary lands at `al-khaser\\Debug\\al-khaser.exe`, with a post-build copy at
  `al-khaser_x86.exe` in the solution root. Equivalent on the command line:
  `msbuild /m /p:Platform=x86 /p:Configuration=Debug al-khaser.sln`. al-khaser
  builds one executable containing every check; individual checks are selected
  at runtime, not at build time.
- **Rationale for Debug over Release**: Debug guarantees per-function symbols are retained (no LTCG inlining). Channel 2 evaluation can additionally test against a Release build to exercise optimization-resilience.
...
- **Identity**: NSIS dropper, 2022 corpus, "fully evasive" sample from a published evasive-malware corpus
- **Status**: rejected
- **Reason**: NSIS wrapper containing Salvity polymorphic file infector as inner payload. Inner PE has runtime-mutating code, so statically recorded offsets do not correspond to stable static-analysis addresses. Polymorphic file infectors are out of scope for v1 fixtures regardless of wrapper.
...
- **Status**: deferred — useful for Channel 2 wrapper-traversal testing
...
- **Status**: rejected
- **Reason**: not freely downloadable. VirusTotal Academic feed required for access. Three free-tier sources (VX Underground search, MalwareBazaar, Hybrid Analysis) returned no hits.
...
- **Status**: deferred — useful for Channel 2 helper-router-pattern testing
...
Real-malware evaluation moves to Channel 2 piloting, where outputs are graded qualitatively against published reverse-engineering writeups rather than against hand-built `expected.json` oracles. The deferred samples above (`269aff53...`, `68644c...`) are the test set for that work.
...
4. **Where a corpus records offsets, verify they land in code, not data.** Such offsets are typically a return address relative to PE base. Adding the image base (typically `0x00400000`) and looking at the instruction one line above the result should reveal a recognizable IAT call to the named API. If it doesn't, either the sample is packed/wrapped (and those offsets are from a runtime memory image that doesn't match the static binary) or BN didn't auto-disassemble the region.
