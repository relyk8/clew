...
def _unresolved(**kw) -> BridgedCallSite:
    base = dict(
        ...
        call_site_va=0x434D4A,
        function_va=0x434D20,
        api_resolution="import",
        parameter_index=-1,
        value=None,
        string_source=None,
...
def test_record_json_round_trips():
    rec = pipeline.assemble_record(
        ...
        bridge_candidates=_candidates(_resolved()),
    )
...
def _find_schema():
    ...
    for cand in (
        here.parent.parent / "schema" / "clew_record.schema.json",
